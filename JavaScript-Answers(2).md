...
* 单页面应用(SPA) -- 核心是“无刷新”，整个 web 应用只在初始化时加载一个导航页及静态资源，之后的操作不再页面的刷新。

* 目标：旨在用为用户提供了更接近移动 APP 或桌面应用程序的体验

* 流程：第一次请求时，将导航页传输到客户端。之后，各功能页通过 JS 操作 URL HASH 或者 History API 进行路由切换，利用 ajax 或 fetch 异步获取 REST API 接口 JSON 数据

* 优点：交互流畅，前后端职责分离，减轻服务器压力，后端接口可复用
...
* 网站重构：不改变UI的情况下，对网站进行优化，在扩展的同时保持一致的 UI
