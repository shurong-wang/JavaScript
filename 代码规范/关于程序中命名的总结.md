...
1. 代码命名是件很头疼的事情，又要`语义化`又要`简洁`，最终目的为了代码的`可维护性`

2. 合适的命名会使代码读起来优雅流畅、耐人寻味，把你的代码变成艺术品。比如：

	* `promise` - 异步请求数据时，“承诺”一定会回应（成功或失败），很有意境
	* `on`、`off` - jquery 中表示监听和销毁事件，言简意赅，简单易用
...
1. 使用能够表达意图的名字
	...
	名字得能告诉我们它要做什么，为什么存在，以及是如何工作的。选择能够表达意图的名字，更有利于我们理解代码。相反，名不副实会误导阅读代码的人。

2. 不要怕在选择名字上花时间

	多尝试几种不同的名字，直至足以描述程序含义，不要害怕在命名上花时间。以后阅读你代码的人（包括自己）会因此受益。
	此外，一个描述性的名称，还有助于你在心中理清模块的设计。良好的命名可能需要花费时间，但是从长远来看，利大于弊。
...
5. 方法名
	...
	方法名应该是一个动词或动词词组，如 postPayment()、deletePage()、save()。访问器和设置器应该分别前缀 get 和 set。返回布尔值的方法应该前缀 `is`，如 isPostable()。
